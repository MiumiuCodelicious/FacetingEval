...
int main(int argc, char **argv) {
    FILE *f;
    char *bestw[N];
    char file_name[max_size], phrases[max_size*2+1], p1[100][max_size], p2[100][max_size];
    float dist, len, bestd[N], vec1[max_size], vec2[max_size];
    long long a, b, c, d, cn1, cn2, bi1[100], bi2[100];
    char ch;
    ...
    if (argc < 2) {
        ...
        printf("No classification will be performed. \n");
    }else if (argc == 3){
//        strcpy(word_cluster_file, argv[2]);
		strcpy(phrases, argv[2]);
...
//        while (1) {
        ...
        for (a = 0; a < cn1; a++) {
            ...
            if (b == words) b = -1;
            bi1[a] = b;
            printf("\nWord 1: %s  Position in vocabulary: %lld\n", p1[a], bi1[a]);
            if (b == -1) {
        ...
        for (a = 0; a < cn2; a++) {
            ...
            if (b == words) b = -1;
            bi2[a] = b;
            printf("\nWord 2: %s  Position in vocabulary: %lld\n", p2[a], bi2[a]);
            if (b == -1) {
                printf("Out of dictionary word!\n");
                //        break;
            }
